...
class LogAnalyzer:
    ...
    def read_logs(
        ...
        # Normalise since/until to timezone-aware UTC
            ...
            try:
                with open_func(file_path, open_mode, encoding='utf-8') as f:
                    for line in f:
                        ...
                        if ts_raw and (since or until):
                            try:
                                log_time = self._parse_timestamp(ts_raw)
                            except Exception:
                                continue
                        ...
                        if level:
                            entry_priority = level_priority.get(
                                log_entry.get('level', 'DEBUG'), 0
                            )
    ...
    def analyze_errors(self, hours: int = 24, group_by: str = 'message') -> Dict[str, Any]:
        """Analyze error logs"""
        since = datetime.now(timezone.utc) - timedelta(hours=hours)

        error_logs = self.read_logs(since=since, level='ERROR')
        ...
        analysis = {
            ...
            'error_distribution': {k: len(v) for k, v in groups.items()},
            'most_common_errors': [],
    ...
    def performance_report(self, hours: int = 1) -> Dict[str, Any]:
        ...
        for log in logs:
            if 'operation' in log and 'duration_ms' in log:
                operations[log['operation']].append(log['duration_ms'])

        performance = {}
        for operation, durations in operations.items():
            if durations:
                sorted_d = sorted(durations)
                p95_index = min(int(len(sorted_d) * 0.95), len(sorted_d) - 1)
                performance[operation] = {
                    'count': len(durations),
                    'avg_ms': sum(durations) / len(durations),
                    'min_ms': min(durations),
                    'max_ms': max(durations),
                    'p95_ms': sorted_d[p95_index],
                }
    ...
    def search_logs(self, query: str, hours: int = 24, case_sensitive: bool = False) -> List[Dict[str, Any]]:
    ...
    def export_logs(self, output_file: str, hours: int = 24, format: str = 'json') -> None:
        """Export logs to file"""
        since = datetime.now(timezone.utc) - timedelta(hours=hours)
        logs = self.read_logs(since=since)
        ...
        for log_file in self.log_dir.glob("*.log.*"):
            if log_file.suffix == '.gz':
                try:
                    ...
                    for part in parts:
                        try:
                            file_date = datetime.strptime(part, '%Y-%m-%d').replace(
                                tzinfo=timezone.utc
                            )
                            if file_date < cutoff:
                                log_file.unlink()

...
class StatisticalDetector:
    ...
    def detect_anomalies(self, df: pd.DataFrame, metric_cols: List[str]) -> pd.DataFrame:
        ...
        anomaly_cols = [
            f'{col}_anomaly' for col in metric_cols if f'{col}_anomaly' in result_df.columns
        ]
        if anomaly_cols:
            result_df['is_anomaly'] = result_df[anomaly_cols].any(axis=1)

            zscore_cols = [
                f'{col}_zscore' for col in metric_cols if f'{col}_zscore' in result_df.columns
            ]
            if zscore_cols:
    ...
    def detect_clusters(
        ...
        if 'is_anomaly' not in df.columns:
            df = self.detect_anomalies(
                df, ['temperature', 'heart_rate', 'activity_level']
            )

        anomalies = df[df['is_anomaly']].copy()

        # Early exit: not enough anomalies to form a cluster
        if len(anomalies) < min_cluster_size:
            return []

        # Group by farm and time window
        anomalies['time_window'] = pd.to_datetime(
            anomalies['date']
